...
public class ProfessorMapper implements ProfessorDAO{
	...
	public void insertProfessor(ProfessorVO professorVO) {
		String sql = "insert into professor(pr_num, pr_name, pr_state, pr_de_num, pr_tell) values(?, ?, ?, ?, ?)";
		...
		try {
			...
			dbConnector.pstmt.setString(5, professorVO.getPr_tell());
	...
	public void updateProfessor(ProfessorVO professorVO) {
		String sql = "update professor set pr_name = ?, pr_state = ?, pr_de_num = ? , pr_tell = ? where pr_num = ?";
		...
		try {
			...
			dbConnector.pstmt.setString(4, professorVO.getPr_tell());
			int count = dbConnector.pstmt.executeUpdate();
			if(count == 0) 
