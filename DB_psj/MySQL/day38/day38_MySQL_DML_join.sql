...
select 속성명들 
    ...
    [order by절]
    [limit절]
- 속성명1과 속성명2가 이름이 같으면 반드시 테이블명을 입력해야함.
- 속성명1과 속성명2가 이름이 다르다면 테이블명을 생략할 수 있다. 
...
-- abc 회원이 구매한 제품명을 조회
select * from `order`;
select distinct or_me_id as 구매자, pr_title as 제품명
	from `order`
...
select distinct or_me_id as 회원, ab_address as 배송지
	from `order`
    join address_book on or_ab_num = ab_num;
    
-- 의류로 등록된 모든 제품과 옵션들을 조회
select * from main_category;
select mc_name as 대분류, pr_title as 제품명, po_title as 옵션  
	from product 
...
-- 분류별 등록된 제품명을 조회
select distinct mc_name as 대분류, sc_name as 소분류, ifnull(pr_title, '등록된 제품 없음') as 제품명
	from product 
