-- 강사님과 함께
drop database if exists shoppingmalltc; -- -> 생략 가능
create database if not exists shoppingmalltc;
use shoppingmalltc;
-- 개발자마다 MySQL table 이름을 짖는 것이 조금씩 다르다.
-- 외래키가 없는 table부터 만드는 것이 좋다.
...
create table if not exists product(
    ...
    pr_pc_num int not null, 				-- 제품 카테고리 번호
...
create table if not exists board(
    ...
    bo_reg_date datetime not null default now(),	-- 게시글 일시(now()->현재시간 자동으로 출력)
