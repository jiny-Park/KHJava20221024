...
public class StringEx01 {
	...
	public static void main(String[] args) {
		 ...
		 * length() : 문자열 길이
		 */
		System.out.println("문자열의 길이 : " + str.length());	//메소드는 () 붙는다.
		int [] arr = new int[5];	
		System.out.println("배열의 길이 : " + arr.length);		//메소드가 아닌 것은 ()가 붙지 않는다. 
		
		/*
		 * indexOf(문자또는 문자열) : 해당 문자열 또는 문자는 0번지부터 첫번째로 만나는 문자열 또는 문자의 시작 번지. 없는 경우 -1
		 * lastIndexOf(문자또는 문자열) : 해당 문자열 또는 문자는 마지막 번지부터 탐색하여 첫번째로 만나는 문자열 또는 문자의 시작 번지. 없는 경우 -1
		 * contains((문자또는 문자열) : 해당 문자열 또는 문자가 있는지 없는지를 알려줌.
 		 */
		System.out.println("e의 첫번째 위치 : " + str.indexOf("e"));
		System.out.println("e의 마지막번째 위치 : " + str.lastIndexOf("e"));
		
		/*
		 * subString(시작반지) : 문자열의 시작번지부터 끝가지 부분 문자열을 추출
		 * subString(시박전지, 마지막번지) : 문자열의 시작번지부터 마지막번지 -1까지 부분 문자열를 추출합니다.
		 */
		 ...
		 */
		String fruit = "사과,배,수박,귤";
		String []fruits = fruit.split(",");
		for(String tmp : fruits) {
			System.out.println(tmp);
		}
		
		/* trim() : 시작 문자 앞에 있는 공백을 제거하고 마지막 문자 뒤에 이는 공백을 제거함.
		 * 
		 */
		String str5 = "\n\n      \t안녕 하세요.\n\n";
